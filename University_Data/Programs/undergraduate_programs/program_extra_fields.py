...
output_dir = os.path.join(current_dir, "Undergrad_prog_outputs")
...
class ProgramDetails(BaseModel):
    ...
    Level: str = Field("Bachelors", description="Program level (e.g. Bachelors, Minor, Associate).")
    ...
    IsAnalyticalNotRequired: bool = Field(False)
    IsAnalyticalOptional: bool = Field(False)
    IsStemProgram: bool = Field(False, description="Is this an official STEM program?")
    IsACTRequired: bool = Field(False, description="Is ACT required?")
    IsSATRequired: bool = Field(False, description="Is SAT required?")
    ...
    GreOrGmat: Optional[str] = Field(None, description="False for undergrad")
    ...
    MinimumACTScore: Optional[float] = Field(None)
    MinimumSATScore: Optional[float] = Field(None)

def process_single_program_master(program_name, program_url, university_name, max_retries=3):
    """
    Mass extraction for a single undergraduate program. Handles Description, STEM status, App Reqs, Test Scores, and Costs.
    """
    from google.genai.types import GenerateContentConfig, Tool, GoogleSearch
    from robust_extraction_utils import _extract_json_from_text
    
    try:
        # Prompt expanded to handle detailed admission and score requirements
        refine_prompt = (
            f"You are a strict data extractor for the undergraduate program '{program_name}' at {university_name}.\n"
            ...
            f"2. If the specific program page fails or is empty, use the main university Undergraduate Programs/Admissions index to find general requirements.\n"
            ...
            f"- 'Resume', 'StatementOfPurpose', 'WritingSample', 'EnglishScore': Set to 'Required' or 'Not Required'.\n"
            ...
            f"- 'IsACTRequired', 'IsSATRequired', 'IsDuoLingoRequired', 'IsELSRequired', 'IsIELTSRequired', 'IsPTERequired', 'IsTOEFLIBRequired', 'IsTOEFLPBTRequired', 'IsEnglishNotRequired', 'IsEnglishOptional': Boolean (true/false).\n"
            f"- 'MinimumACTScore', 'MinimumSATScore', 'MinimumDuoLingoScore', 'MinimumELSScore', 'MinimumIELTSScore', 'MinimumPTEScore', 'MinimumTOEFLScore', 'MinGPA': Numerical value (float).\n"
        ...
        for attempt in range(max_retries):
             try:
                 ...
                 # Final mapping ensures all keys exist in output
                 result = {
                     ...
                     'IsACTRequired': parsed.get('IsACTRequired', False),
                     'IsSATRequired': parsed.get('IsSATRequired', False),
                     ...
                     'MinimumACTScore': parsed.get('MinimumACTScore'),
                     'MinimumSATScore': parsed.get('MinimumSATScore'),
                 ...
                 bool_fields = [
                     'IsStemProgram', 'IsACTRequired', 'IsSATRequired', 'IsDuoLingoRequired', 
                     'IsELSRequired', 'IsGMATOrGreRequired', 'IsGMATRequired', 'IsGRERequired', 
...
def run(university_name_input):
    ...
    if os.path.exists(master_json_path):
        try:
             ...
             yield '{"status": "progress", "message": "Resuming: Loaded {len(master_data)} existing master records."}'
        except Exception:
             pass
    ...
    if programs_to_process:
         yield f'{{"status": "progress", "message": "Starting parallel master extraction for {len(programs_to_process)} undergraduate programs. Please wait..."}}'
    ...
    # Output expanded keys to satisfied frontend/merge scripts
    step2_keys = [
        'Program name', 'Program Page url', 'description', 'IsStemProgram', 'ProgramCategory',
        'Concentration name', 'Resume', 'StatementOfPurpose', 'WritingSample', 'EnglishScore',
        'IsACTRequired', 'IsSATRequired', 'MinimumACTScore', 'MinimumSATScore',
    ...
    yield f'{{"status": "complete", "message": "Master extraction complete.", "files": {{"undergrad_extra_csv": "{step2_csv_path}"}}}}'